...
public class ToastManager : MonoBehaviour {
    ...
    // for pauseable toast
    public static bool isPauseToast = false;
    ...
    Subscription<ToastRequest> toast_request_subscription;
    ...
    void Awake()
    {
        ...
        toast_request_subscription = EventBus.Subscribe<ToastRequest>(_OnToastRequest);
    }
    ...
    void _OnToastRequest(ToastRequest e)
    {
        instance.requests.Enqueue(e);
    }
    ...
    void Update()
    {
        ...
        if (!toasting && requests.Count > 0)
        {
            ...
            instance.show_duration = new_request.duration;
            instance.StartCoroutine(DoToast(instance.ease_duration, instance.show_duration, new_request.isPauseable, 
                                            new_request.isTaskComplete, new_request.isStoryUnlock));
        }
    ...
    IEnumerator DoToast(float duration_ease_sec, float duration_show_sec, bool withPause = false, bool taskComplete = false, bool unlockStory = false)
    {
        Vector3 initialPosition = instance.toast_panel.transform.position;
        Vector3 final_position = Vector2.zero;
        if (unlockStory)
        {
            final_position = GameObject.FindGameObjectWithTag("StoryButton").transform.position;
        }
        if (withPause)
        {
            isPauseToast = true;
        ...
        EventBus.Publish<RemoveLine>(new RemoveLine());
        // I add this to fix a bug -- this isn't in origin code
        if (taskComplete) EventBus.Publish<ToastCrossOut>(new ToastCrossOut());
        ...
        if (unlockStory)
        {
            yield return StartCoroutine(MoveObjectOverTime(instance.toast_panel.transform, instance.toast_panel.transform.position,
                       new Vector3(initialPosition.x, Screen.height / 1.5f, 1), 1f, true));
        }
        else
        {
            while (progress < 1.0f)
        ...
        // Specialines for pauseable content;
        while (withPause && isPauseToast)
        {
        ...
        else
        {
            yield return StartCoroutine(
                MoveObjectOverTime(instance.toast_panel.transform, instance.toast_panel.transform.position, final_position, 1f, false)
            );
            instance.toast_panel.transform.position = initialPosition;
            instance.toast_panel.transform.localScale = Vector3.one;
        }
    ...
    {
        EventBus.Unsubscribe<ToastRequest>(toast_request_subscription);
    }

    IEnumerator MoveObjectOverTime(Transform target, Vector3 initial_pos, Vector3 dest_pos, float duration_sec, bool isFirst)
    {
        float initial_time = Time.time;
        ...
        while (progress < 1.0f)
        {
            ...
            if (isFirst)
            {
                Vector3 new_scale = Vector3.Lerp(target.transform.localScale, new Vector3(1.5f, 1.5f, 1), progress);
                target.localScale = new_scale;
            }
            else
            {
                Vector3 new_scale = Vector3.Lerp(new Vector3(1.5f, 1.5f, 1), new Vector3(0.2f, 0.2f, 1), progress);
                target.localScale = new_scale;
            }
